...
export default class DisappearCommand extends Command {
    ...
    public async interactionRun(interaction: CommandInteraction<AnyTextChannelWithoutGroup | Uncached>): Promise<void> {
        ...
        const data = await api.get("gif", "disappear")
        ...
        if(user){
            if(interaction.user.mention === user.mention){
                ...
                interaction.createMessage({
                    content: `${user.mention} disappeared by ${interaction.user.mention}`,
                    embeds: [
                        {
                            author: {
                                name: user.username,
